...
2.  **本地拦截（优化）**：如果当前进程设置了 `PF_FREE_PAGES` 标志（通常发生在 `balance_classzone` 的直接回收过程中），释放的页框会被拦截并挂载到进程私有的 `local_pages` 链表上，用于“自给自足”(减少全局锁竞争, 但并非目的)。

> **Note: 与 Slab Per-CPU Caches 的区别**
> 虽然两者都利用了“本地化”思想，但 **Slab Per-CPU Caches** 是为了减少多核 CPU 间的锁竞争（对象级），而 **PF_FREE_PAGES** 是为了优化“直接回收”路径，确保回收进程能优先使用自己释放的页框（页框级），避免“白忙活”。
